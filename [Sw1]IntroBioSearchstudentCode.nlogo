...
;;We need each robot to know some information about itself.
 robots-own [
   ...
   siteX
   siteY
   ]           ;;robots need to know if they are in the searching state.
 ...
 to setup 
   ...
   ;; targetPatches is a temporary variable
   ...
   set targetPatches (clusterOfRocks * 5)
      while[targetPatches > 0][
         ask one-of patches[
             if pcolor != black and pcolor != yellow
                and [pcolor] of neighbors4 != black and [pcolor]
                of neighbors4 != yellow[
                    set pcolor yellow
   ...
   set numberOfRocks (numberOfRocks + (clusterOfRocks * 5))
